...
#%% Method implementation - IPM approach

############################## Method train ##################################

# Input: train data of shape [Y, T, X, R]
# Output: trained model 

def MTRNetIPM(data_train, y_cont, hyperparams):
    ...
    # Model 
    class RepresentationIPM(nn.Module):
        def __init__(self, x_size, layer_size_rep, drop):
            super(RepresentationIPM, self).__init__()
            self.model = nn.Sequential(
                nn.Linear(x_size, layer_size_rep),  
                nn.ELU(),
                nn.Dropout(drop),
    ...
    class HypothesisT0_IPM(nn.Module):
        def __init__(self, layer_size_rep, layer_size_hyp, y_size, drop):
            super(HypothesisT0_IPM, self).__init__()
            self.model = nn.Sequential(
                nn.Linear(layer_size_rep, layer_size_hyp),  
                nn.ELU(),
                nn.Dropout(drop),
    ...
    class HypothesisT1_IPM(nn.Module):
        def __init__(self, layer_size_rep, layer_size_hyp, y_size, drop):
            super(HypothesisT1_IPM, self).__init__()
            self.model = nn.Sequential(
                nn.Linear(layer_size_rep, layer_size_hyp),  
                nn.ELU(),
                nn.Dropout(drop),
    ...
    class X_predIPM(nn.Module):
        def __init__(self, layer_size_rep, x_size, drop):
            super(X_predIPM, self).__init__()
            self.model = nn.Sequential(
                nn.Linear(layer_size_rep, layer_size_rep),
                nn.ELU(),
                nn.Dropout(drop),
    ...
    class MTRNetIPM(nn.Module):
        def __init__(self, x_size, layer_size_rep, layer_size_hyp, y_size, drop):
            super(MTRNetIPM, self).__init__()
            self.representation = RepresentationIPM(x_size, layer_size_rep, drop)
            self.hypothesisT0 = HypothesisT0_IPM(layer_size_rep, layer_size_hyp, y_size, drop)
            self.hypothesisT1 = HypothesisT1_IPM(layer_size_rep, layer_size_hyp, y_size, drop)
            self.x_pred = X_predIPM(layer_size_rep, x_size, drop)
        ...
        def forward(self, x):
            ...
            y1_out = self.hypothesisT1(rep)
            x_out = self.x_pred(rep)
            return y0_out, y1_out, x_out, rep
    
    mod_MTRNetIPM = MTRNetIPM(x_size=p, layer_size_rep=layer_size_rep, 
                              layer_size_hyp=layer_size_hyp, y_size=y_size, drop=drop).to(device)
    optimizer = torch.optim.Adam([{'params': mod_MTRNetIPM.representation.parameters(), 'weight_decay': 0},
                                  {'params': mod_MTRNetIPM.hypothesisT0.parameters()},
                                  {'params': mod_MTRNetIPM.hypothesisT1.parameters()}],
                                 lr=lr, weight_decay=lam)  
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.97)   
    MSE_Y = nn.MSELoss(reduction='none') # For continuous Y
    Entropy_Y = nn.CrossEntropyLoss(reduction='none') # For categorical Y
    ...
    for iteration in range(n_iter):
        ...
        # Forward pass
        y0_out, y1_out, x_out, rep = mod_MTRNetIPM(x_data)
        
        # Loss 
        idxR0 = torch.where(r_data == 0) 
        idxR1 = torch.where(r_data == 1)
        idxR1T0 = torch.where((r_data == 1) & (t_data == 0)) 
        idxR1T1 = torch.where((r_data == 1) & (t_data == 1))
        if y_size == 1:
            ...
            lossY = torch.mean(wt_data[idxR1T0]*Entropy_Y(y0_out[idxR1T0], y_data[idxR1T0])) + \
                torch.mean(wt_data[idxR1T1]*Entropy_Y(y1_out[idxR1T1], y_data[idxR1T1])) # For categorical Y
        n_iter_Wass = 10
        n_control = idxR1T0[0].shape[0]
        n_treated = idxR1T1[0].shape[0]
        if np.min([n_control,n_treated]) > 0:
            ...
            lossT = 0
        n_missing = idxR0[0].shape[0]
        n_observed = idxR1[0].shape[0]
        if np.min([n_missing,n_observed]) > 0:
            rep_R0 = rep[idxR0]
            rep_R1 = rep[idxR1]
            Mr = torch.empty(n_observed, n_missing)
            for i in range(n_observed):
                for j in range(n_missing):
                    Mr[i,j] = torch.sqrt(torch.sum(torch.square(rep_R1[i,:] - rep_R0[j,:])))
            lam_Wass = 10
            Kr = torch.exp(-lam_Wass*Mr)
            ur = (1/n_observed)*torch.ones(n_observed,1)
            for i in range(n_iter_Wass):
                ur = n_observed/(torch.matmul(n_missing*Kr,(1/torch.matmul(torch.transpose(Kr,0,1),ur))))
            vr = 1/torch.matmul(torch.transpose(Kr,0,1),ur)  
            Tr = torch.matmul(torch.diag(ur.view(n_observed)), 
                              torch.matmul(Kr, torch.diag(vr.view(n_missing))))
            lossR = torch.sum(Tr*Mr)
        else:
            lossR = 0
        lossX = MSE(x_out, x_data)
        loss = lossY + alpha*lossT + beta*lossR + gamma*lossX
    ...
    return mod_MTRNetIPM

# mod_MTRNetIPM = MTRNetIPM(d_train[:,1:])
...
def MTRNetIPM_pred(data_test, model, y_cont):
    data_test = torch.from_numpy(data_test.astype(np.float32))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    data_test = data_test.to(device)
